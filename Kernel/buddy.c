...
void printBuddyLists(){
    ...
    while(i<=MAX_MEM_POWER){
        printf(" AVAIL[%d] --> AVAIL[i].order = %d --> AVAIL[%d].available = %d --> AVAIL[%d].next = %p --> AVAIL[%d].prev = %p ADDRESS = %p \n ",i,AVAIL[i].order,i,AVAIL[i].available,i,AVAIL[i].next,i,AVAIL[i].prev,&AVAIL[i]);
        i++;
    }
}