...
int start_proc_user(char *procName, void *procPointer, int argc, char* argv[]) {
    return int80((uint64_t) procName, (uint64_t) procPointer, (uint64_t) argc, (uint64_t) argv, 0, SYS_NEW_PROC);
}
...
int *joinByPipe(char *readerProcName, void *readerProcPointer, char *writerProcName, void *writerProcPointer){
  ...
  int writerPid = start_proc_user(writerProcName, writerProcPointer, 0, 0);
  switchFd(STDOUT, STDOUT);
  switchFd(STDIN, pipe[0]);
  int readerPid = start_proc_user(readerProcName, readerProcPointer, 0, 0);
